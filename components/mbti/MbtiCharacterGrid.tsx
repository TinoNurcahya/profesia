    ...
    <div ref={rootRef}>
      ...
      <div className="mbti-desktop-scene page-shell"><a href="#mbti-role-index" className="atlas-text-link mb-4 text-sm">{t("skipStory")}</a>
        <div data-mbti-scene className="mbti-group-scene" aria-hidden="true">
          {roleKeys.map((role, index) => <div key={role} data-mbti-group className="mbti-group-frame"><div><p className={`atlas-caption ${roleInk[role]}`}>{labels.roleProgress.replace("{current}", String(index + 1)).replace("{total}", "4")}</p><p className="atlas-heading mt-6">{roleLabels[role]}</p><p className="atlas-body mt-6 max-w-md">{labels.scrollHint}</p></div><div className="grid grid-cols-2 gap-4">{types.filter(type => type.group_en === role).map(type => <div key={type.code} className="border-b border-[var(--color-line)]"><div className="relative h-44"><Image src={`/images/mbti/${type.code.toLowerCase()}.png`} alt="" fill sizes="200px" className="object-contain" /></div><p className="atlas-caption py-3">{type.code} / {isEnglish ? type.name_en : type.name_id}</p></div>)}</div></div>)}
        </div>
      </div>
